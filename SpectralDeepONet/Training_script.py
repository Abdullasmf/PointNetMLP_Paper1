...
from benchmarks import SpectralDeepONet
...
def main(preset_name: str = "S0", batch=8, dataset: str = "L_bracket") -> None:
    ...
    # DeepONet / Spectral Config extraction directly from JSON (_cfg)
    ...
    # 3. Fourier Params (Spectral)
    # Uses 'head_posenc' dict from JSON if available
    spec_n_freqs = 10
    spec_scale = 2.0
    if isinstance(head_posenc, dict):
         spec_n_freqs = int(head_posenc.get("n_freqs", 10))
         spec_scale = float(head_posenc.get("scale", 2.0))
